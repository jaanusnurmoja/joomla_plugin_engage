    ...
    public function add_engage_identifier($token){
        ...
        if ($ident = $auth_info->profile->identifier) { // valid ident?
            ...
            if (isset($exist_ident) && $exist_ident == $user->id) {
                JError::raiseError("auth_error",'This social identifier is already enabled on this account.');
            } elseif (isset($exist_ident) && $exist_ident != $user->id) {
                JError::raiseError("auth_error",'This social identifier is associated with another account on this website.');
